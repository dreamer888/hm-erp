...
class ExpressMenuConfig(models.Model):
    ...
    name = fields.Char(string='快递公司名称', required=True)
    abbreviation = fields.Char(string='简称', required=True)
    customername = fields.Char(string='客户名称')
    customerpwd = fields.Char(string='客户密码')
    monthcode = fields.Char(string='月结号')
    sendsite = fields.Char(string='发送站点')
    logisticcode = fields.Char(string='快递单号')
...
class Warehouse(models.Model):
    ...
    detail_address = fields.Char(string='详细地址')
    principal_id = fields.Many2one('staff', string='负责人')
    # 用于外键关联到staff这张表，当前负责人
...
class WhMove(models.Model):
    ...
    express_menu = fields.Text(string='快递面单', copy=False)
    ...
    def get_shipping_type_config(self, menu_type):
        ...
        if not express_menu:
           raise UserError(f'单据{self.name},承运商暂不支持 或者承运商商户简称输入错误{menu_type}')
    ...
    # 在获取快递单号时，因为我们用到的是测试接口，所以不管收件人是谁，快递鸟都只会返回一个相同的快递单号
    def md5(self,n):
        md5 = hashlib.md5()
