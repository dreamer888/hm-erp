...
class MoneyInvoice(models.Model):
    ...
    cogs_voucher_id = fields.Many2one('voucher', u'成本凭证', readonly=True, ondelete='set null',
                                 copy=False,
                                 help=u'结算单确认时生成的成本凭证')
    ...
    def money_invoice_draft(self):
        ...
        for invoice in self:
            ...
            if invoice.is_init:
                ...
                voucher.unlink()
            # 删除发出成本凭证
            cogs_voucher = self.env['voucher'].search([('ref','=','%s,%s' % (self._name, invoice.id))])
            if cogs_voucher:
    ...
    def money_invoice_done(self):
        ...
        for invoice in self:
            ...
            if invoice.category_id.type == 'income':
                ...
                send_voucher = False
                if self.env['ir.module.module'].sudo().search([('name', '=', 'sell'), ('state', '=', 'installed')]):
                    delivery = self.env['sell.delivery'].search([('name','=',invoice.name)])
                    send_voucher = delivery.voucher_id
                # 该公司不是月末一次性结转发出成本
                if send_voucher:
                    cost_voucher_data = send_voucher.copy_data()
                    out_account_id = self.env.ref('finance.categ_sell_goods').account_id.id
                    cogs_account_id = self.env.company.cogs_account.id
                    # 如果发货单没有直接结转成本
                    if out_account_id != cogs_account_id:
                        for l in cost_voucher_data[0]['line_ids']:
                            if l[2]['account_id'] != out_account_id:
                                l[2].update({'account_id':cogs_account_id})
                            l[2].update({'debit':l[2]['credit'],
                                         'credit':l[2]['debit'],
                                         'voucher_id':False})
                        cost_voucher_data[0].update({'ref': '%s,%s' % (self._name, self.id)})
                        if invoice.invoice_date:
                            cost_voucher_data[0].update({'date': invoice.invoice_date})
                        cogs_voucher = self.env['voucher'].create(cost_voucher_data)
