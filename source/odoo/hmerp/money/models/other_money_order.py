##############################################################################
#
#    Copyright (C) 2016  唤梦科技(<http://www.dreammm.net>).
#
...
class OtherMoneyOrder(models.Model):
    ...
    def _compute_total_amount(self):
        ...
        for omo in self:
            ...
            omo.total_amount = sum((line.amount + line.tax_amount)
                                for line in omo.line_ids)
    ...
    def _compute_details(self):
        for v in self:
            vl = {'col': [], 'val': []}
            vl['col'] = ['类别', '会计科目', '辅助核算', '金额']
            for l in v.line_ids:
                vl['val'].append([
                    l.category_id.name,
                    l.account_id.display_name,
                    l.auxiliary_id.name or '',
                    l.amount])
    ...
    def other_get_create_voucher_line(self, vouch_obj, init_obj):
        ...
        :return:
        """
        in_currency_id = self.bank_id.currency_id.id or self.env.user.company_id.currency_id.id
        company_currency_id = self.env.user.company_id.currency_id.id
        in_is_company_currency = in_currency_id == company_currency_id
        vals = {}
        for line in self.line_ids:
            ...
            vals.update({'vouch_obj_id': vouch_obj.id, 'name': self.name, 'note': line.note or '',
                         ...
                         'currency_amount': not in_is_company_currency and self.total_amount or 0, #判断是否本币
            ...
            if not init_obj:
                self.env['voucher.line'].create({
                    ...
                    'credit': in_is_company_currency and line.amount or line.amount * vals.get('rate_silent'),
                    'voucher_id': vals.get('vouch_obj_id'),
                    'auxiliary_id': vals.get('credit_auxiliary_id', False),
                    'currency_amount': not in_is_company_currency and line.amount or 0,
            ...
            if vals.get('sell_tax_amount'):
                ...
                self.env['voucher.line'].create({
                    ...
                    'credit': in_is_company_currency and line.tax_amount or line.tax_amount * vals.get('rate_silent') or 0,
                    'voucher_id': vals.get('vouch_obj_id'),
                    'currency_amount': not in_is_company_currency and line.tax_amount or 0,
                    'rate_silent': vals.get('rate_silent'),
        ...
        self.env['voucher.line'].create({
            ...
            'debit': in_is_company_currency and self.total_amount or self.total_amount * vals.get('rate_silent'),  # 借方和
            'voucher_id': vals.get('vouch_obj_id'),
    ...
    def other_pay_create_voucher_line(self, vouch_obj):
        ...
        :return:
        """
        out_currency_id = self.bank_id.currency_id.id or self.env.user.company_id.currency_id.id
        company_currency_id = self.env.user.company_id.currency_id.id
        out_is_company_currency = out_currency_id == company_currency_id
        vals = {}
        for line in self.line_ids:
            if not line.category_id.account_id:
                raise UserError(u'请配置%s的会计科目' % (line.category_id.name))
            ...
            vals.update({'vouch_obj_id': vouch_obj.id, 'name': self.name, 'note': line.note or '',
                         ...
                         'currency_id': self.bank_id.currency_id.id,
                         'currency_amount': not out_is_company_currency and self.total_amount or 0, #判断是否本币
                         'rate_silent': rate_silent,
                         })
            # 借方行
            self.env['voucher.line'].create({
                'name': u"%s %s " % (vals.get('name'), vals.get('note')),
                'account_id': vals.get('debit_account_id'),
                'debit': out_is_company_currency and line.amount or line.amount * vals.get('rate_silent'),
                'voucher_id': vals.get('vouch_obj_id'),
                'partner_id': vals.get('partner_debit', ''),
                'auxiliary_id': vals.get('debit_auxiliary_id', False),
                'init_obj': vals.get('init_obj', False),
                'currency_amount': not out_is_company_currency and line.amount or 0,
                'rate_silent': vals.get('rate_silent'),
            })
            # 进项税行
            if vals.get('buy_tax_amount'):
                ...
                self.env['voucher.line'].create({
                    ...
                    'debit': out_is_company_currency and line.tax_amount or line.tax_amount * vals.get('rate_silent') or 0,
                    'voucher_id': vals.get('vouch_obj_id'),
                    'currency_amount': not out_is_company_currency and line.tax_amount or 0,
                    'rate_silent': vals.get('rate_silent'),
                })
        # 贷方行
        self.env['voucher.line'].create({
            ...
            'partner_id': vals.get('partner_credit', ''),
            'account_id': vals.get('credit_account_id'),
            'credit': out_is_company_currency and self.total_amount or self.total_amount * vals.get('rate_silent'),  # 借方和
            'voucher_id': vals.get('vouch_obj_id'),
            'auxiliary_id': vals.get('credit_auxiliary_id', False),
            'init_obj': vals.get('init_obj', False),
            'currency_id': vals.get('currency_id', False),
...
class OtherMoneyOrderLine(models.Model):
    ...
    account_id = fields.Many2one('finance.account',
                                '科目',related='category_id.account_id')
