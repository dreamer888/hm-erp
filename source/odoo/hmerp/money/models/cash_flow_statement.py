...
class CashFlowTemplate(models.Model):
    ...
    # for type begin
    begin_ids = fields.Many2many('finance.account', relation='cftb', string=u'会计科目期初')
    # for type end
    end_ids = fields.Many2many('finance.account', relation='cfte', string=u'会计科目期末')
    # for type lines
...
class CashFlowStatement(models.Model):
    ...
    name = fields.Char(u'项目')
    line_num = fields.Char(u'行次')
    amount = fields.Float(u'本月金额', digits='Amount')
    year_amount = fields.Float(u'本年累计金额', digits='Amount')
