...
class MoneyTransferOrder(models.Model):
    ...
    def money_transfer_done(self):
        ...
        for line in self.line_ids:
            ...
            out_currency_id = line.out_bank_id.currency_id.id or company_currency_id
            in_currency_id = line.in_bank_id.currency_id.id or company_currency_id
            ...
            if out_currency_id == company_currency_id:  # 如果转出账户是公司本位币
                ...
                if in_currency_id == company_currency_id:   # 如果转入账户是公司本位币
                    line.in_bank_id.balance += line.amount
                    line.out_bank_id.balance -= line.currency_amount
                elif in_currency_id != company_currency_id and in_currency_id == out_currency_id:   # 如果转入账户是外币,只支持同种外币互转
                    line.in_bank_id.balance += line.currency_amount
                    line.out_bank_id.balance -= line.currency_amount
                else:
    ...
    def money_transfer_draft(self):
        ...
        for line in self.line_ids:
            ...
            if line.currency_amount > 0:
                if in_currency_id != company_currency_id and out_currency_id == in_currency_id:  # 转入和转出都是外币
                    if float_compare(line.in_bank_id.balance, line.currency_amount, precision_digits=decimal_amount.digits) == -1:
                        ...
                        line.in_bank_id.balance -= line.currency_amount
                        line.out_bank_id.balance += line.currency_amount
                elif in_currency_id == company_currency_id and out_currency_id != company_currency_id:   # 转入账户为本位币,转出是外币
                    if float_compare(line.in_bank_id.balance, line.amount, precision_digits=decimal_amount.digits) == -1:
                        ...
                        line.in_bank_id.balance -= line.amount
                        line.out_bank_id.balance += line.currency_amount
                elif in_currency_id != company_currency_id and out_currency_id == company_currency_id:   # 转入账户为外币,转出是人民币
                    if float_compare(line.in_bank_id.balance, line.currency_amount, precision_digits=decimal_amount.digits) == -1:
                        raise UserError('转入账户余额不足。\n转入账户余额:%s 本次转出金额:%s'
                                        % (line.in_bank_id.balance, line.currency_amount))
                    else:
    ...
    def create_voucher(self):
        ...
        vals = {}
        for line in self.line_ids:
            out_currency_id = line.out_bank_id.currency_id.id or self.env.user.company_id.currency_id.id
            in_currency_id = line.in_bank_id.currency_id.id or self.env.user.company_id.currency_id.id
            company_currency_id = self.env.user.company_id.currency_id.id
            if (
            ...
            if line.currency_amount and in_currency_id == company_currency_id and out_currency_id != company_currency_id:
                '''结汇'''
                '''借方行'''
            ...
            elif line.currency_amount and in_currency_id != company_currency_id and out_currency_id == company_currency_id:
                '''买汇'''
                '''借方行'''
                self.env['voucher.line'].create({
            ...
            elif line.currency_amount and in_currency_id == out_currency_id and in_currency_id != company_currency_id:
                '''相同外币币别互转'''
                '''借方行'''
                rate_silent = self.env['res.currency'].get_rate_silent(self.date, in_currency_id)
                self.env['voucher.line'].create({
                    'name': u"%s %s外币互转至%s %s" % (self.name, line.out_bank_id.name, line.in_bank_id.name, self.note),
                    'account_id': line.in_bank_id.account_id.id, 'debit': line.currency_amount * rate_silent,
                    'voucher_id': vouch_obj.id, 'partner_id': '', 'currency_id': in_currency_id,
                    'currency_amount': line.currency_amount, 'rate_silent': rate_silent
                })
                '''贷方行'''
                self.env['voucher.line'].create({
                    'name': u"%s %s外币互转至%s %s" % (self.name, line.out_bank_id.name, line.in_bank_id.name, self.note),
                    'account_id': line.out_bank_id.account_id.id, 'credit': line.currency_amount * rate_silent,
                    'voucher_id': vouch_obj.id, 'partner_id': '', 'currency_id': out_currency_id,
                    'currency_amount': line.currency_amount, 'rate_silent': rate_silent
