...
class CashFlowWizard(models.TransientModel):
    ...
    def get_amount(self, tem, report_ids, period_id):
        ...
        if tem.line_type == 'category':
            # 其他收支单金额合计
            ret = sum([line.amount + line.tax_amount 
                      for line in self.env['other.money.order.line'].search(
                          [('category_id', 'in', [c.id for c in tem.category_ids]),
