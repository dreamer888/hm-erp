...
class TrialBalance(models.Model):
    ...
    def _get_year_init(self):
        for tb in self:
            ...
            if tb.subject_name_id.balance_directions == 'in':
                # 年初借 = 期末借 - 期末贷 - 本年借 + 本年贷
                tb.year_init_debit = tb.ending_balance_debit - tb.ending_balance_credit - \
                    tb.cumulative_occurrence_debit + tb.cumulative_occurrence_credit
                tb.year_init_credit = 0
            else:
                # 年初贷 = 期末贷 - 期末借 - 本年贷 + 本年借
                tb.year_init_credit = tb.ending_balance_credit - tb.ending_balance_debit - \
                    tb.cumulative_occurrence_credit + tb.cumulative_occurrence_debit
    ...
    year_init_debit = fields.Float('年初余额(借方)', digits='Amount', default=0, compute=_get_year_init)
    year_init_credit = fields.Float('年初余额(贷方)', digits='Amount', default=0, compute=_get_year_init)
...
class CheckTrialBalanceWizard(models.TransientModel):
    ...
    total_year_init_debit = fields.Float('年初余额(借方)', digits='Amount', default=0)
    total_year_init_credit = fields.Float('年初余额(贷方)', digits='Amount', default=0)
