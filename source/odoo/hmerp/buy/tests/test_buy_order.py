...
class TestBuyOrder(TransactionCase):
    ...
    def test_get_buy_goods_state_part_in(self):
        ...
        back_rec = self.env['buy.receipt'].search(
            [('order_id', '=', self.order.id), ('state', '=', 'draft')])
        back_rec.action_cancel()
        self.assertTrue(self.order.goods_state == '部分入库剩余作废')
...
class TestBuyOrderLine(TransactionCase):
    ...
    def test_compute_all_amount_foreign_currency(self):
        '''外币测试：当订单行的数量、含税单价、折扣额、税率改变时，改变采购金额、税额、价税合计'''
        # 单据上外币是公司本位币
