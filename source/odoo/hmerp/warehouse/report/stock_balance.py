...
class ReportStockBalance(models.Model):
    ...
    cost = fields.Float('成本', digits='Price')
        ...
        cr.execute(
            ...
            create or replace view report_stock_balance as (
                SELECT min(line.id) as id,
                       ...
                       line.lot as lot,
                       line.expiration_date as expiration_date,
                ...
                GROUP BY wh.name, line.lot, line.expiration_date, attribute.name, goods.name, goods.id, goods.brand, loc.name, uom.name, uos.name
