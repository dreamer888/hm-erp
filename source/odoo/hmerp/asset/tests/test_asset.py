...
class TestAsset(TransactionCase):
    ...
    def test_unlink(self):
        '''测试删除已审核的固定资产'''
        asset = self.asset.copy({'code': '001copy'})
    ...
    def test_asset_draft_change_line(self):
        ...
        for line in self.asset.chang_ids:
            self.assertTrue(line.period_id)
        with self.assertRaises(UserError):
            self.asset.asset_draft()
    ...
    def test_asset_draft_after_invoice_paid(self):
        self.asset.asset_done()
        self.asset.money_invoice.reconciled = 6
        with self.assertRaises(UserError):
            self.asset.asset_draft()
    ...
    def test_asset_draft_bank_account(self):
        ...
        self.asset.onchange_bank_account()
        self.asset.asset_done()
        self.asset.other_money_order.other_money_done()
        self.asset.asset_draft()
    ...
    def test_get_cost_depreciation(self):
        ...
        wizard = self.env['create.depreciation.wizard'].create({'date': '2016-06-01'})
        wizard.create_depreciation()

    def test_no_depreciation(self):
        ''' 不提折旧 '''
        self.asset.no_depreciation = True
        self.asset._get_cost_depreciation()
...
class TestCreateCleanWizard(TransactionCase):
    ...
    def test_compute_period_id(self):
        ...
        wizard._compute_period_id()
        wizard.create_clean_account()   # 不传context执行不成功
        wizard.with_context({'active_id': self.asset.id}).create_clean_account()
        self.assertTrue(self.asset.net_value == 0)
...
class TestDepreciationWizard(TransactionCase):
    ...
    def test_period_not_there(self):
        self.env['finance.period'].get_date_now_period_id().unlink()
        with self.assertRaises(UserError):
            # 当前日期对应的会计期间不存在，请先创建！
            self.env['create.depreciation.wizard'].create({})
    ...
    def test_create_depreciation_account_diff(self):
        ''' 多个固定资产生成凭证 '''
        asset_2 = self.asset.copy({'code': '001copyagain'})
        asset_2.asset_done()
        self.asset.asset_done()
        wizard = self.env['create.depreciation.wizard'].create({'date': '2016-05-01'})
        wizard.create_depreciation()
